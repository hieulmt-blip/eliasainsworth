...
async def rebalance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [
            InlineKeyboardButton("➖➖ Sell Only", callback_data="rb_sell"),
            InlineKeyboardButton("➖➕ Sell → Buy", callback_data="rb_full"),
            InlineKeyboardButton("➕➕ Buy Only", callback_data="rb_buy"),
        ]
    ]

    await update.message.reply_text(
        "⚖️ CHỌN MODE REBALANCE",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def rebalance_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()

    mode = query.data

    ratios = update_and_get_capital_ratios()
    portfolio, nav = get_portfolio_value()

    if not ratios:
        await query.message.reply_text("⚠️ Sheet chưa có ratio.")
        return

    msg = "⚖️ REBALANCE RESULT\n\n"

    # ===== MODE -+ (Rebalance thuần) =====
    if mode == "rb_full":
        for coin, target_pct in ratios.items():
            target_value = (target_pct / 100) * nav
            current_value = portfolio.get(coin, 0)
            ...
            if diff > 0:
                msg += f"BUY {coin} ≈ {diff:.2f} USDT\n"
            else:
                msg += f"SELL {coin} ≈ {abs(diff):.2f} USDT\n"
